    ...
    if req.method == 'POST':
        ...
        if fm.is_valid():
            ...
            if(pp.count() > 1):
                ...
                if(candidate and interviewer):
                    for p in pp:
                        schedule = Interview.objects.filter(
                            participants__in=[p])
                        for slot in schedule:
                            ...
                            if (slot.date == mdate or slot.date+timedelta(days=1) == mdate) and overlap(farg, sarg):
                                ...
                                messages.warning(req, msg.format(
                                    p.name, slot.start_time, slot.end_time))
                ...
                if time and candidate and interviewer:
                    ...
                    messages.warning(
                        req, 'At least 1 Interviewer and 1 Candidate is required')
            else:
